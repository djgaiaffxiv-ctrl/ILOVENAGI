'use strict';
// ILOVESIRI - Gestor de dependencias externas (Ghostscript, qpdf, LibreOffice, Tesseract).
...
function httpsGet(url, headers) {
  return new Promise((resolve, reject) => {
    https.get(url, { headers: { 'User-Agent': 'ilovesiri', ...(headers || {}) } }, (res) => {
...
async function installGhostscript(onLog) {
  try {
    ...
    const installer = path.join(os.tmpdir(), 'ilovesiri-gs-setup.exe');
    ...
    return { ok: false, message: 'Ghostscript se descargo pero no se detecto. Reinicia ILOVESIRI.' };
  } catch (e) {
