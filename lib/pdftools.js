'use strict';
// ILOVESIRI - Implementacion de todas las herramientas PDF.
...
function tmpDir() {
  const d = path.join(os.tmpdir(), 'ilovesiri-' + Date.now() + '-' + Math.floor(Math.random() * 1e6));
  fs.mkdirSync(d, { recursive: true });
  return d;
}

function run(cmd, args, { onLog, cwd } = {}) {
...
async function merge(inputs, opts = {}) {
  ...
  const output = uniquePath(opts.output || path.join(path.dirname(inputs[0]), 'ILOVESIRI-unido.pdf'));
...
async function watermark(input, opts = {}) {
  const text = opts.text || 'ILOVESIRI';
  const doc = await PDFDocument.load(await fsp.readFile(input), { ignoreEncryption: true });
...
async function imagesToPdf(inputs, opts = {}) {
  ...
  const output = uniquePath(opts.output || path.join(path.dirname(inputs[0]), 'ILOVESIRI-imagenes.pdf'));
  await fsp.writeFile(output, await doc.save());
...
async function ocr(input, opts = {}, ctx = {}) {
  ...
  for (let i = 0; i < imgs.length; i++) {
    ...
    await run(tesseract, [imgs[i], outBase, '-l', lang, 'pdf'], ctx);
...
async function unprotectExcel(input, opts = {}) {
  const ext = path.extname(input).toLowerCase();
  if (ext !== '.xlsx' && ext !== '.xlsm') {
    throw new Error('Solo .xlsx / .xlsm. Para .xls antiguo, abrelo y guardalo como .xlsx primero.');
  }
  let zip;
  try { zip = new AdmZip(input); }
  catch (e) { throw new Error('No se pudo abrir el Excel (¿esta cifrado con contrasena de apertura? Eso necesita la clave).'); }

  let removed = 0;
  const reSheet = /<sheetProtection[^>]*\/>/g;
  const reBook = /<workbookProtection[^>]*\/>/g;
  const reShare = /<fileSharing[^>]*\/>/g;
  zip.getEntries().forEach(entry => {
    const name = entry.entryName;
    if (!name.endsWith('.xml')) return;
    if (!(name.startsWith('xl/worksheets/') || name === 'xl/workbook.xml')) return;
  ...
  const output = uniquePath(opts.output || outName(input, '-desprotegido'));
  zip.writeZip(output);
  return {
    outputs: [output],
    message: removed
      ? 'Excel desprotegido: se quitaron ' + removed + ' bloqueo(s) de hoja/libro.'
      : 'No se encontro proteccion de hoja/libro (puede que solo tenga contrasena de apertura, que requiere la clave).'
  };
}
