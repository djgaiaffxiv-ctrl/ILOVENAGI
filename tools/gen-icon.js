'use strict';
// Genera el icono de ILOVENAGI: logo NAGI (transparente) + titulo en acento de app.
// Usa el Electron ya instalado para rasterizar (no requiere ImageMagick).
// Salida: build/icon.png (256), build/icon.ico (multi-tamano), build/icon-512.png, src/logo.png
...
const ACCENT = '#EE9A2E';   // ambar -> acento de ILOVENAGI
const TITLE  = 'ILOVENAGI';
...
<style>
 ...
 .title{font-family:'Rubik','Segoe UI',system-ui,sans-serif;font-weight:900;font-size:72px;letter-spacing:1.5px;color:${ACCENT};line-height:1}
</style></head>
<body><div class="wrap">
...
async function main() {
  ...
  console.log('captura:', sz.width + 'x' + sz.height, '| vacia:', img.isEmpty());
  ...
  fs.writeFileSync(path.join(SRC, 'logo.png'), png256);
  ...
  console.log('OK -> build/icon.ico, build/icon.png, src/logo.png');
  app.quit();
}
